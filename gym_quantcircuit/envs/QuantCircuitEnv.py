...
# QiskitRuntimeService.save_account(channel='ibm_quantum', instance='ibm-q/open/main', token='aa3ac15ea20cbbd7904f04af5b8fd32e22ba2a083539582fca3b5658becb75e5642a6b9e54b3bbbceae582924c5ff3c0e24273b148193f986576a3b1618f2286')
        ...
        self.EPS = 1e-10
        self.has_run = False
        self.goal_accuracy = 1.0
        self.previous_accuracy=0
        self.current_accuracy=0
        self.objective_func_vals = []
        self.max_gates = self.action_space_n
        print("gate_list: ",self.gate_list)
    ...
    def reset(self):
        ...
        # Reset the agent's accuracy tracking
        #diff = self.goal_accuracy - self.previous_accuracy
        ...
        self.step_count = 0
        self.gate_count = 0
        # Resetta altri attributi se necessario
        # Restituisci lo stato iniziale e altre info
        diff=self.goal_accuracy-self.previous_accuracy
        info = {'current_accuracy': 0.0, 'circuit': self.qcircuit}
        return diff, info
    ...
    def step(self, action):
        ...
        # Estrai il gate e i qubit associati dall'action
        gate_qubits = self.gate_list[action][0]
        gate_function = self.gate_list[action][1]

        # Controlla se il gate è multi-qubit o single-qubit
        if len(gate_qubits) > 1:
            # Multi-qubit gate (ad esempio CNOT)
            control_qubit = gate_qubits[0]
            target_qubit = gate_qubits[1]
            gate_function(self.q_reg[control_qubit], self.q_reg[target_qubit])
        else:
            # Single-qubit gate
            single_qubit = gate_qubits[0]

            # Se è un gate di rotazione, genera un angolo casuale
            if gate_function in [self.qcircuit.rx, self.qcircuit.ry, self.qcircuit.rz]:
                theta = np.random.uniform(0, 2 * np.pi)  # Genera un angolo casuale
                gate_function(theta, self.q_reg[single_qubit])
            else:
        ...
        if self.step_count >= 1 and self.step_count <= 10:
            accuracy = self.evaluate_circuit()
            self.has_run=True

            # Applica una funzione sigmoide all'accuratezza per ottenere la reward
            reward = 1 / (1 + np.exp(-10 * (accuracy - 0.85)))  # Sigmoide centrata intorno a 0.98
        ...
        assert action in range(self.action_space_n), 'Azione non valida.'

        done = False
        self.step_count += 1
        self.gate_count += 1

        # Estrai il gate e i qubit associati dall'azione
        gate_qubits = self.gate_list[action][0]
        gate_function = self.gate_list[action][1]

        # Applica il gate al circuito
        if len(gate_qubits) > 1:
            # Gate multi-qubit
            gate_function(*gate_qubits)
        else:
            # Gate single-qubit
            if gate_function in [self.qcircuit.rx, self.qcircuit.ry, self.qcircuit.rz]:
                theta = np.random.uniform(0, 2 * np.pi)
                gate_function(theta, gate_qubits[0])
            else:
        ...
        if self.step_count >= 1 and self.step_count <= 5:
            ...
            if accuracy >= self.goal_accuracy:
                ...
                print("Episodio terminato: accuratezza raggiunta.")
                diff = self.goal_accuracy - accuracy
            elif self.step_count >= 5:
                done = True
                print("Episodio terminato: limite di passi raggiunto.")
                diff = self.goal_accuracy - accuracy
            else:
                diff = self.goal_accuracy - accuracy
        else:
            # Per step_count fuori dall'intervallo 1-10, definisci come desideri
        ...
        reward = float(reward)
        measures = {'fidelity': round(self.fidelity(), 3)}

        info = {'current_accuracy': accuracy, 'circuit': self.qcircuit}
        return diff, reward, done, measures
    ...
    def set_gate_group(self, gate_group, custom_gates=None):
        ...
        if gate_group == 'clifford':
            ...
            self.gate_group_list = [
                ((0, 1), self.qcircuit.cx),  # CNOT gate - multi-qubit
                ((0,), self.qcircuit.h),     # Hadamard gate - single qubit
                ((0,), self.qcircuit.rx),    # RX gate - single qubit
                ((0,), self.qcircuit.ry),    # RY gate - single qubit
                ((0,), self.qcircuit.rz)     # RZ gate - single qubit
            ]
    ...
    def _create_gates(self):
        ...
        for gates in self.gate_group_list:
            ...
            if isinstance(gates, tuple):
                gate_qubits = len(gates[0])  # Prendi il numero di qubit dalla tupla
                gates = gates[1]
            else:
    ...
    def evaluate_circuit(self) -> bool:
        """print("Circuito : ")
        print(self.qcircuit)
        print("Feature map : ")
        ...
        error = depolarizing_error(0.05, 1)
        noise_model.add_all_qubit_quantum_error(error,instructions=['u1', 'u2', 'u3','h','z','i','id'])
        transpiled=transpile(self.qcircuit,backend=AerSimulator(noise_model=noise_model),optimization_level=3)
        vqc = VQC(
        ...
        train_score_q4 = vqc.score(self.X_train, self.Y_train)
        test_score_q4 = vqc.score(self.X_test, self.Y_test)

        print(f"Quantum VQC on the training dataset: {train_score_q4:.2f}")
        print(f"Quantum VQC on the test dataset:     {test_score_q4:.2f}")
        return test_score_q4
        """
        """X=self.X_train
        ...
        if self.has_run:
            ...
            scores = cross_val_score(qsvc, X, y, cv=5)
